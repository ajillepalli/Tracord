...
def shared_expectation_validator(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
...
def test_parse_requires_exact_case_name():
    assert_error(descriptor(), "assertion_case_not_found", case_name="SMOKE")


@pytest.mark.parametrize(
    ("data", "code"),
    [
        (b"\xef\xbb\xbf{}", "assertion_file_invalid_utf8"),
        (b"\xff", "assertion_file_invalid_utf8"),
        (b"{", "assertion_file_invalid_json"),
        (b'{"schema_version":"x","schema_version":"y","cases":{}}', "assertion_file_invalid_json"),
...
def test_parse_rejects_invalid_top_level_shape(document: object, location: str | None):
    assert_error(json.dumps(document).encode(), "assertion_file_invalid", location)


def test_parse_rejects_more_than_256_cases():
    cases = {f"case-{index:03d}": {"status": "passed"} for index in range(257)}
    assert_error(descriptor(cases), "assertion_file_invalid", "cases")
...
def test_parse_rejects_nonportable_case_names(name: str):
    assert_error(descriptor({name: {"status": "passed"}}), "assertion_file_invalid", "cases")
...
def test_parse_rejects_ineffective_or_invalid_expectations(
    ...
    assert_error(descriptor({"smoke": case}), "assertion_file_invalid", location)
...
def test_parse_uses_shared_expectation_validation(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        assertion_files.assertions,
        "validate_expectations",
        lambda expectations: seen.append(expectations) or ["invalid"],
    )

    assert_error(
        descriptor(), "assertion_file_invalid", "cases.smoke"
    )
    assert len(seen) == 1


class FakeIdentityResult(Enum):
    SAME = auto()
    DIFFERENT = auto()
    UNKNOWN = auto()
...
@pytest.fixture
def path_api(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(assertion_files.paths, "IdentityResult", FakeIdentityResult, raising=False)
    monkeypatch.setattr(
        assertion_files.paths, "check_containment", lambda root, path: None, raising=False
    )
    monkeypatch.setattr(
        assertion_files.paths,
        "stat_regular_no_follow",
    ...
    monkeypatch.setattr(
        assertion_files.paths,
        "same_file_snapshot",
        lambda first, second: FakeIdentityResult.SAME,
        raising=False,
    )


def test_load_reads_a_regular_single_link_file_safely(tmp_path: Path, path_api: None):
...
def test_load_reports_missing_without_disclosing_path(tmp_path: Path, path_api: None):
    secret_path = tmp_path / "secret-name.json"
    with pytest.raises(AssertionFileError) as caught:
        load_assertion_case(secret_path, "smoke")

    assert caught.value.code == "assertion_file_not_found"
    assert str(secret_path) not in str(caught.value)


def test_load_rejects_oversized_file(tmp_path: Path, path_api: None):
    path = tmp_path / "assertions.json"
...
def test_load_fails_closed_when_snapshot_identity_is_unknown(
    tmp_path: Path, path_api: None, monkeypatch: pytest.MonkeyPatch
):
    path = tmp_path / "assertions.json"
    path.write_bytes(descriptor())
    monkeypatch.setattr(
        assertion_files.paths,
        "same_file_snapshot",
        lambda first, second: FakeIdentityResult.UNKNOWN,
    )

    with pytest.raises(AssertionFileError) as caught:
        load_assertion_case(path, "smoke")
    assert caught.value.code == "assertion_file_unsafe"
...
def test_error_rejects_unapproved_codes_and_unsafe_locations():
    ...
    with pytest.raises(ValueError):
        AssertionFileError("assertion_file_invalid", "cases.secret value")
